...
def main():
    ...
    parser.add_argument("-t", "--target", dest="target", required=False, help="vcf file of target SNPs")
    ...
    if args.target != None:
        with open (args.target, "r") as file:
            lines = file.readlines()
            file2 = open(gkm_folder+'/input_snp.tsv', "w")
            for line in lines:
                if (line.startswith('#')) or (line.startswith('ID')):
                    continue 
                snpID, chrom, pos, ref, alt = line.rstrip().split("\t")[0:5]
                file2.write("{0}_chr{1}_{2}_{3}_{4}\n".format(snpID, chrom, pos, ref, alt))
            file2.close()
    ...
    for tf in tfs:
        ...
        subprocess.call(gkm_folder+'/scripts/gkmpredict -T 4 '+gkm_folder+'/data/selex_allelic_oligos.ref.fa '+gkm_folder+'/gkmsvm_models/' + model + ' '+gkm_folder+'/out/' + tf + '.ref.gkm.tsv &>'+gkm_folder+'/log/gkmpredict.ref.log', shell=True)
        subprocess.call(gkm_folder+'/scripts/gkmpredict -T 4 '+gkm_folder+'/data/selex_allelic_oligos.alt.fa '+gkm_folder+'/gkmsvm_models/' + model + ' '+gkm_folder+'/out/' + tf + '.alt.gkm.tsv &>'+gkm_folder+'/log/gkmpredict.ref.log', shell=True)
