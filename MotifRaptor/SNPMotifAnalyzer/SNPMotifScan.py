...
def write_gkm_scale_file(motif_scan_folder, outputmotifscandir, motif_id):
    motif_score_filename=os.path.join(motif_scan_folder, motif_id+".scores")
    motif_scale_filename=os.path.join(outputmotifscandir, motif_id+".scale")
    scale_file=open(motif_scale_filename,"w")
    score_df=pd.read_csv(motif_score_filename,sep="\t")
    score_df_sub=score_df[(score_df['binding_ref']>0)&(score_df['binding_alt']>0)]
    max_binding_score=max(max(score_df['binding_ref']),max(score_df['binding_alt']))
    max_disrupt_score=max(score_df['deltaSVM_score'])
    headstring="bg_max_bind_score"+"\t"+"bg_max_disrupt_score"+"\t"+"theoretic_max_bind_score"+"\t"+"theoretic_max_disrupt_score"
    scale_file.write(headstring+"\n")
    outstring=str(max_binding_score)+"\t"+str(max_disrupt_score)+"\t"+str(max_binding_score)+"\t"+str(max_disrupt_score)
    scale_file.write(outstring+"\n")
    scale_file.close()


def run_gkm_scale(motif_scan_folder, outputmotifscandir, numberofthreads):
    ext = [".scores"]
    motif_id_list = []
    for file in os.listdir(motif_scan_folder):
        if file.endswith(tuple(ext)):
...
def run_snp_motif_gkm(target_snp_filename, background_snp_filename, motif_list_filename, motif_expression_filename, gkm_score_folder, motif_scan_folder, output_filename, num_of_threads):
    df_snps_valid=pd.read_csv(target_snp_filename, sep='\t')
    df_snps_valid['UID']=df_snps_valid['ID']+":"+df_snps_valid['REF']+":"+df_snps_valid['ALT']
    #df_snps_valid.set_index('ID', inplace=True)
    ...
    p=multiprocessing.Pool(processes=num_of_threads)
    getSNPMotif_func=partial(getSNPMotif_gkm, df_snps_valid, background_snp_id_list, df_motif_express, gkm_score_folder, motif_scan_folder)
    pool_results=p.map(getSNPMotif_func, motif_id_list)
    p.close()
    p.join()
    # merging parts processed by different processes
...
def getSNPMotif_gkm(df_snps_valid, background_snp_id_list, df_motif_express, gkm_score_folder, motif_scan_folder, motif_id):
    ...
    for valid_index in df_snps_valid.index:
        ...
        disruption_area_score, disruption_score, ref_part, alt_part= calculate_area_on_the_fly_gkm(currentsnp, gkm_score_folder, motif_id)
        if ref_part==(0,0,0) or alt_part==(0,0,0):
            ref_part=(0,0,-1.1111)
            alt_part=(0,0,-1.1111)
        #if float(ref_part[2])<0 and float(alt_part[2])<0:
...
def calculate_area_on_the_fly_gkm(currentSNP, gkm_score_folder, motif_id):
    motif_len=10
    motif_hits_ref = extract_all_motifs_triplets_gkm(gkm_score_folder, motif_id, motif_len, currentSNP.seq_ref, currentSNP.pos_start_ref,currentSNP.pos_end_ref)
    motif_hits_alt = extract_all_motifs_triplets_gkm(gkm_score_folder, motif_id, motif_len, currentSNP.seq_alt, currentSNP.pos_start_alt,currentSNP.pos_end_alt)
    disruption_area_score=0
    disruption_score=0
    reference_part=(0,0,0)
    alternative_part=(0,0,0)
...
def extract_all_motifs_triplets_gkm(gkm_score_folder, motif_id, motif_len, sequence, snp_start_pos,snp_end_pos):
    min_start=max(min(snp_start_pos-motif_len+1,snp_end_pos-motif_len+1),0)
    max_start=min(max(snp_start_pos,snp_end_pos),len(sequence)-1)
    output_dict=dict()
    output_dict=motif_scan_gkm(sequence, motif_id, gkm_score_folder, motif_len, min_start, max_start, "+", output_dict)
    output_dict=motif_scan_gkm(sequence, motif_id, gkm_score_folder, motif_len, min_start, max_start, "-", output_dict)
    return output_dict

def motif_scan_gkm(sequence, motif_id, gkm_score_folder, motif_len, pos_start, pos_stop, strand, output_dict):
    if output_dict==None:
        output_dict=dict()
    seq_len=len(sequence)
    sequence=sequence.upper()
    if strand=="+":
        for start_pos in range(pos_start, pos_stop+1):
            if start_pos+motif_len>seq_len:
                break
            this_sequence=sequence[start_pos:start_pos+motif_len]
            score=sequence_motif_score_gkm(this_sequence, motif_id, gkm_score_folder)
            output_dict[start_pos+1]=(start_pos,strand,score)
    elif strand=="-":
        old_chars = "ACGTYSMBDRWKVHN"
        replace_chars = "TGCARWKVHYSMBDN"
...
def sequence_motif_score_gkm(sequence, motif_id, gkm_score_folder):
    score=0
    column=0
...
def main():
    parser = argparse.ArgumentParser(prog='SNPMotifScan', description='Scan motif on SNPs on the fly.')
    subparsers = parser.add_subparsers(help='help for subcommand: scale, pfmscan, bindscan, gkm_scale, gkm_scan', dest="command")
    ...
    # create the parser for the "gkm_scale" command
    parser_d = subparsers.add_parser('gkm_scale', help='gkm_scale help')
    parser_d.add_argument('-score', '--score_folder', type=str, help='motif score files folder',dest="score_folder")
    parser_d.add_argument('-mo', '--motifscan_output', type=str, help='Motif Scan Ouput Folder',dest="outputmotifscandir")
    parser_d.add_argument('-p', '--threads', type=int, help='number of threads',dest="thread_num")


    # create the parser for the "gkm_scan" command
    parser_e = subparsers.add_parser('gkm_scan', help='scan motif from gkm files')
    parser_e.add_argument('-t', '--target_snp', type=str, help='target snp list file with sequences',dest="target_snps")
    parser_e.add_argument('-bg', '--bg_snp', type=str, help='background snp list file or (genome)',dest="bg_snps")
    parser_e.add_argument('-m', '--motifs', type=str, help='motif list file, no header',dest="motif_list")
    parser_e.add_argument('-e', '--expression', type=str, help='expression file',dest="expression")
    parser_e.add_argument('-gkm', '--gkm_folder', type=str, help='gkm_score folder',dest="gkm_folder")
    parser_e.add_argument('-score', '--score_folder', type=str, help='motif score files folder (with scaling parameters)',dest="score_folder")
    parser_e.add_argument('-mo', '--motifscan_output', type=str, action='store', default='result_new_df.txt', help='SNPs with Motif Scan Ouput',dest="snp_motif_out")
    parser_e.add_argument('-p', '--threads', type=int, help='number of threads',dest="thread_num")
    ...
    elif args.command=="gkm_scale":
        print("Command: generating scale files...")
        run_gkm_scale(args.score_folder, args.outputmotifscandir, args.thread_num)
    ...
    elif args.command=="gkm_scan":
        print("Command: scanning...")
        run_snp_motif_gkm(args.target_snps,args.bg_snps,args.motif_list,args.expression,args.gkm_folder,args.score_folder,args.snp_motif_out,args.thread_num)
...
if __name__=="__main__":
    #sys.argv = "SNPMotifScan.py scale -pfm pfmfiles -score motifscanfiles -p 2"
    main()